...
cat("Sum of all predictions on response scale:", sum(preds_out), "this should be 17472. \n\n")

cat("Sum of all predictions on link scale:", sum(preds_out_link), "this should be -65738.33. \n\n")

cat("Time for training:", total_time) #9.006237

salsa2dlog <- read.table('salsa2d.log')
n_fits <- 0
for (ii in 1:nrow(salsa2dlog)){
  row_in <- salsa2dlog[rw]
...
cat("Number of fits:", n_fits, "Original fits: ")
...
cat("Sum of all estimates:", sum(sum_results$coefficients[,1]), "this should be -1.929935. \n\n")

cat("Sum of all robust SE:", sum(sum_results$coefficients[,3]), "this should be 55.14092. \n\n")

cat("Standard errors are equal to robust standard errors:", sum(sum_results$coefficients[,2])==sum(sum_results$coefficients[,3]), "\n\n")

cat("Dispersion:", sum_results$dispersion, "this should be 35.85788. \n\n")
...
cat("Mean square error to truth:", mean_sum_sq, "this should be 0.4298723. \n\n")
...
cat("Mean square error to truth seen:", mean_sum_sq_seen, "this should be 0.4298723. \n\n")

mean_sum_sq_unseen <- sum((predictData$truth.re[!rows_seen] - preds_out[!rows_seen])^2)/nrow(predictData[!rows_seen,])

cat("Mean square error to truth unseen:", mean_sum_sq_unseen, "this should be 0.4298723. \n\n")
