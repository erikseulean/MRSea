...
## Introduction
This is the sixth in a series of tests that can be used to check changes to the MRSea package do not break previously working elements. They replicate the case study in the vignette but broken down into chunks. This sixth one checks the 2D salsa functions with other variables, with a 1d smooth, no panels and no interactions.
...
predict_read_in <- read.csv("6.predict.data.csv")
...
knot_loc_read <- read.csv("6.knot.loc.csv")
...
for (rr in 1:nrow(knot_loc_read)) {
  pred_link <- pred_link + annotate("path", x=knot_loc_read$xx[rr] + 0.5 * cos(seq(0,2*pi,length.out=100)), y=knot_loc_read$yy[rr] + 0.5 * sin(seq(0,2*pi,length.out=100)))
}
...
cat("Sum of all predictions on response scale:", sum(preds_out), "this should be 2192.313. \n\n")

cat("Sum of all predictions on link scale:", sum(preds_out_link), "this should be -7589.302. \n\n")

cat("Time for training:", total_time, "Original time: 8.3. \n\n")
...
cat("Number of model fits:", n_fits, "Original model fits: 227")
...
```{r, echo=FALSE}
sum_results_read_in <- read.csv("6.sum.results.csv")
...
cat("Sum of all estimates:", sum(sum_results$coefficients[,1]), "this should be -0.2626807. \n\n")

cat("Sum of all robust SE:", sum(sum_results$coefficients[,3]), "this should be 53.9878. \n\n")

cat("Standard errors are equal to robust standard errors:", sum(sum_results$coefficients[,2])==sum(sum_results$coefficients[,3]), "\n\n")

cat("Dispersion:", sum_results$dispersion, "this should be 30.48591. \n\n")
...
cat("Mean square error to truth:", mean_sum_sq, "this should be 0.4155882. \n\n")

mean_sum_sq_data <- sum((data$response - predict.gamMRSea(newdata=data, object=salsa2dOutput$bestModel))^2)/nrow(predictData)

cat("residual sum square error to data:", mean_sum_sq_data, "this should be 11.53858. \n\n")
...
cat("Mean square error to truth seen:", mean_sum_sq_seen, "this should be 0.441726. \n\n")

mean_sum_sq_unseen <- sum((predictData$truth.re[!rows_seen] - preds_out[!rows_seen])^2)/nrow(predictData[!rows_seen,])

cat("Mean square error to truth unseen:", mean_sum_sq_unseen, "this should be 0.4071793. \n\n")
